...
getgenv()['Check_Setting'] = function(Name)
    if not _G.Dis and not isfolder('gus Hub BF Premium') then
        makefolder('gus Hub BF Premium')
    end
    if not _G.Dis and not isfile('gus Hub BF Premium/'..Name..'.json') then
        writefile('gus Hub BF Premium/'..Name..'.json',JsonEncode(_G.Setting_table))
    end
end
getgenv()['Get_Setting'] = function(Name)
    if not _G.Dis and isfolder('gus Hub BF Premium') and isfile('gus Hub BF Premium/'..Name..'.json') then
        _G.Setting_table = JsonDecode(readfile('gus Hub BF Premium/'..Name..'.json'))
...
local win = library:Window("gus",[[Hub]],[[Version : Premium ]],"9606070311",Enum.KeyCode.RightControl)
